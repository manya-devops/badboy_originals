      ...
      <div className="flex justify-between items-start px-0.5">
        ...
        <div className="text-right">
          ...
          {product.originalPrice && (
            <span className="block text-[9px] sm:text-[10px] text-white/20 line-through">₹{(product.originalPrice || 0).toLocaleString('en-IN')}</span>
          )}
        </div>
      </div>
    </motion.div>
        ...
        // map backend → frontend format
          ...
          const formatted = Array.isArray(data)
            ? data.map((item:any) => ({
              ...
              image: item.image || item.image_url || "",
              hoverImage: item.image || item.image_url || "",
              badge: item.discount_percentage
